...
function initShortcodes(container) {
	...
	if (container.find('.sc_toggles_init:not(.inited)').length > 0) {
		container.find('.sc_toggles_init:not(.inited)').each(function () {
			"use strict";
			var init = jQuery(this).data('active');
			if (isNaN(init)) init = 0;
			else init = Math.max(0, init);
			init=init+1;
			var active_child =  jQuery(this).find('.sc_toggl_item:nth-child(' + init + ')');
			active_child.addClass('sc_active');
			active_child.find('.sc_toggl_content').show();
		});
	...
	if (container.find('.sc_form .sc_form_button .sc_button:not(.inited)').length > 0 && container.find('.sc_form').data('formtype') == 'contact' ) {
		container.find('.sc_form .sc_form_button .sc_button:not(.inited)')
			...
			.click(function(e){
				"use strict";
				userSubmitForm(jQuery(this).parents("form"), jQuery(this).parents("form").attr('action'), THEMEREX_ajax_nonce);
				e.preventDefault();
				return false;
			});
	}
	...
	container.find('.sc_slider_swiper:not(.inited)')
		.each(function () {
			...
			jQuery(this).find('li.swiper-slide').each(function(){
				"use strict";
				var x = jQuery(this).data('style');
				jQuery(this).attr('style',x);
			});
				//specify height
			...
			if (button.hasClass('sc_eform_button')) {
				...
				if (!regexp.test(email)) {
					...
					if(jQuery(container).find('.error').length <= 0)
						jQuery(container).append('<div class="error">' + THEMEREX_EMAIL_NOT_VALID + '</div>');
				} else {
